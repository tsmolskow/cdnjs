...
Function GetUserAccessReport($WebAppURL, $FileUrl)
{
    ...
    #“   User Name | User Login | Groups" | Out-file $FileUrl -Append

    $AllSCUsers = Get-SPUser -Web $SiteCollections.Url | Select DisplayName, UserLogin, IsDomainGroup, Groups 
    foreach($SCUser in $AllSCUsers){

        If($SCUser.IsDomainGroup){

            #"      Name: $($SCUser.DisplayName) | Login: $($SCUser.UserLogin) | Domain Group " | Out-file $FileUrl -Append

        } elseif ($SCUser.Groups.Count -eq 0) {

            #"      Name: $($SCUser.DisplayName) | Login: $($SCUser.UserLogin) | Group: $($SCUser.Permission) | Direct Permission " | Out-file $FileUrl -Append

        } else {

            #"      Name: $($SCUser.DisplayName) | Login: $($SCUser.UserLogin) | Groups: $($SCUser.Groups -join ", ") " | Out-file $FileUrl -Append
        }
    ...
    foreach($Site in $SiteCollections)
    {
        ...
        foreach($Web in $Site.AllWebs)
        {
            ...
            #"   User Name | User Login | User's Groups" | Out-File $FileUrl -Append

            $AllSCUsersWeb = Get-SPUser -Web $Web.Url | Select DisplayName, UserLogin, IsDomainGroup, Groups 
            foreach($SCUserWeb in $AllSCUsersWeb){

                If($SCUserWeb.IsDomainGroup){

                    #"      Name: $($SCUserWeb.DisplayName) | Login: $($SCUserWeb.UserLogin) | Domain Group " | Out-file $FileUrl -Append

                } elseif ($SCUserWeb.Groups.Count -eq 0) {

                    #"      Name: $($SCUserWeb.DisplayName) | Login: $($SCUserWeb.UserLogin) | Direct Permission " | Out-file $FileUrl -Append

                } else {

                    #"      Name: $($SCUserWeb.DisplayName) | Login: $($SCUserWeb.UserLogin) | Groups: $($SCUserWeb.Groups -join ", ") " | Out-file $FileUrl -Append
                }
            }
