...
$spoSiteSGConnection = Connect-Site -Url $spoSite -Browser -DisableSSO # Connect to SPO Using Browser
$spopSiteSGConnection = Connect-Site -Url $spopSite -Browser -DisableSSO # Connect to SPOP Using Browser

## Connect to SPO and SPOP Sites Using Prompt
#$Credentials = Get-Credential
#$spoSiteSGConnection = Connect-Site -Url $spoSite -Credential $Credentials # Connect to SPO Using Prompt

## Connect to SPO and SPOP Sites Using Prompt
#$Credentials = Get-Credential
#$spopSiteSGConnection = Connect-Site -Url $spopSite -Credential $Credentials # Connect to SPOP Using Prompt

Write-Host "Reuseable Connections Made `n" -ForegroundColor Green
...
foreach ($row in $table)
{
    ...
    try {
        ...
        ## Connect to SPO using Existing Connection (No Prompt)
        $spoSiteSG = Connect-Site -Url $spoSite -UseCredentialsFrom $spoSiteSGConnection 
        
        ## Connect to SPOP using Browser (Prompt) 
