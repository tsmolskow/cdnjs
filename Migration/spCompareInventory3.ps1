...
[io.file]::readalltext($CSVPath).replace("Content type publishing error log","") | Out-File $CSVPath -Encoding ascii –Force
[io.file]::readalltext($CSVPath).replace("Workflows","") | Out-File $CSVPath -Encoding ascii –Force
