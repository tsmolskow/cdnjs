## spSetUserProfileDataAdmin3.ps1
...
$csvFile = "C:\Scripts\CSVInput\UserProfileDBUpdate.csv"
...
$Title = $row.Title
$SPSSipAddress = $row.'SPS SIP Address'
$WebSite = $row.'Web Site'
$SPSResponsibility = $row.'SPS Responsibility'
$Office = $row.Office
$SPSPictureTimestamp = $row.'Picture Time Stamp'
$SPSPicturePlaceholderState = $row.'Picture Placeholder State'
$SPSPictureExchangeSyncState = $row.'Picture Exchange Sync State'
$Role = $row.Role
...
Write-Host "Profile Summary: Account Name - " $AccountName", First Name Last Name - " $FirstName $LastName"," $Department #For Testing
...
$UserProfileManager = New-Object Microsoft.Office.Server.UserProfiles.UserProfileManager($ServiceContext)
    ...
    if ($UserProfileManager.UserExists($AccountName))
    {
        ...
        write-host "Profile report for:"$profile["AccountName"] "processed successfully! `n" -f Green

    }
    else
    {
        write-host "$($UserLogin) Not Found!" -f Red | Out-File $errorLog -Append
    } 
